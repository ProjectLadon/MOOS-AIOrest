...
class AIOconf {
    public:
        ...
        std::vector<unique_ptr<Publisher>> &getPublishers() {return publishers;};
        std::vector<unique_ptr<Subscriber>> &getSubscribers() {return subscribers;};
        const std::string &getURL() {return url;};

    private:    // nethods
        std::unique_ptr<rapidjson::Document> makeRequest (std::string url, rapidjson::Document *request);
    ...
    private:    // members
        ...
        std::iterator<std::vector<std::unique_ptr<Publisher>>> publishPtr;
};
